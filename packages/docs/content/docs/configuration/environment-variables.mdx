...
| `USENET_MAX_CONCURRENT_INSPECTS`   | Max Concurrent Inspects   | number   | `4`          | How many NZB imports (the availability check that runs before a stream starts or a manual add completes) may run at the same time. Extra imports wait in a queue, and playback-triggered imports are served before background adds. **0** removes the limit.                                                                                                                                           |
