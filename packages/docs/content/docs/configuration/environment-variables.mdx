...
| `USENET_PREFETCH_SEGMENTS`         | Prefetch Segments         | number   | `32`         | How many pieces of the file each stream downloads ahead of the current playback position. Higher values give faster, smoother streaming on a good connection, but use more memory per stream. Only applies when the performance profile is **custom**.                                                                                                                                                 |
