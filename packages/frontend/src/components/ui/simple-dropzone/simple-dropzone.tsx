...
export const SimpleDropzoneAnatomy = defineStyleAnatomy({
  ...
  fileIcon: cva(['UI-SimpleDropzone__fileIcon', 'w-5 h-5 flex-none']),
  maxSizeText: cva([
...
export type SimpleDropzoneProps = Omit<
...
>((props, ref) => {
  ...
  return (
    <BasicField {...basicFieldProps}>
      <button
        ref={buttonRef}
        className={cn(SimpleDropzoneAnatomy.root(), className)}
      ...
      {!withImagePreview && (
        <div className={cn(SimpleDropzoneAnatomy.list(), listClass)}>
          {files?.map((file: any, index) => {
            let Icon: React.ReactElement;

            if (
              ['image/jpeg', 'image/png', 'image/jpg', 'image/webm'].includes(
                file.type
              )
            ) {
              Icon = (
                <svg
                  ...
                  strokeLinejoin="round"
                  className={cn(
                    SimpleDropzoneAnatomy.fileIcon(),
                    fileIconClass
                  )}
                >
            ...
            return (
              ...
              >
                <div
                  className={cn(
                    SimpleDropzoneAnatomy.listItemDetailsContainer(),
                    listItemDetailsContainerClass
                  )}
                >
                  {Icon}
                  <p
                    className={cn(
                      SimpleDropzoneAnatomy.listItemTitle(),
                      listItemTitleClass
                    )}
                  >
                    {file.name}
                  ...
                  <p
                    className={cn(
                      SimpleDropzoneAnatomy.listItemSize(),
                      listItemSizeClass
                    )}
                  >
                ...
                <IconButton
                  ...
                  className={cn(
                    SimpleDropzoneAnatomy.listItemRemoveButton(),
                    listItemRemoveButtonClass
                  )}
      ...
      {withImagePreview && !!files.length && (
        <div
          className={cn(
            SimpleDropzoneAnatomy.imagePreviewGrid(),
            imagePreviewGridClass
          )}
        >
          {files?.map((file, index) => {
            return (
              <div
                key={file.name}
                className={cn(
                  SimpleDropzoneAnatomy.imagePreviewContainer(),
                  imagePreviewContainerClass
                )}
              >
                <div
                  className={cn(
                    SimpleDropzoneAnatomy.imagePreview(),
                    imagePreviewClass
                  )}
                  style={{
                    backgroundImage: file
                      ? `url(${(file as File & { preview: string }).preview})`
                      : undefined,
                  }}
                >
                  <CloseButton
                    ...
                    size="xs"
                    className={cn(
                      SimpleDropzoneAnatomy.imagePreviewRemoveButton(),
                      imagePreviewRemoveButtonClass
                    )}
                    onClick={() => handleRemoveFile(index)}
                  />
